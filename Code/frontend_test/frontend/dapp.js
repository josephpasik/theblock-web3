...
const dApp = {
  ...
  collectVars: async function() {
    // get land tokens
    this.tokens = [];
    this.totalSupply = await this.marketContract.methods.totalSupply().call();
    ...
    for (let i = 1; i <= this.totalSupply; i++) {
      try {
        const token_uri = await this.marketContract.methods.tokenURI(i).call();
        console.log('token uri', token_uri)
        const token_json = await fetchMetadata(token_uri);
        console.log('token json', token_json)
        this.tokens.push({
          tokenId: i,
          highestBid: Number(await this.marketContract.methods.highestBid(i).call()),
          auctionEnded: Boolean(await this.marketContract.methods.auctionEnded(i).call()),
          pendingReturn: Number(await this.marketContract.methods.pendingReturn(i, this.accounts[0]).call()),
          auction: new window.web3.eth.Contract(
            this.auctionJson,
            await this.marketContract.methods.auctions(i).call(),
            { defaultAccount: this.accounts[0] }
          ),
          owner: await this.marketContract.methods.ownerOf(i).call(),
          ...token_json
        });
      } catch (e) {
  ...
  bid: async function(event) {
    ...
    await this.marketContract.methods.bid(tokenId).send({from: this.accounts[0], value: wei}, async () => {
      await this.updateUI();
    });
  },
  endAuction: async function(event) {
    const tokenId = $(event.target).attr("id");
    await this.marketContract.methods.endAuction(tokenId).send({from: this.accounts[0]}, async () => {
      await this.updateUI();
    });
  },
  ...
  registerLand: async function() {
    ...
    try {
      ...
      await this.marketContract.methods.registerLand(reference_uri).send({from: this.accounts[0]}, async () => {
  ...
  main: async function() {
    ...
    this.marketJson = await (await fetch("./BrokerlessMarket.json")).json();
    this.auctionJson = await (await fetch("./BrokerlessAuction.json")).json();
    this.depositJson = await (await fetch("./Deposit.json")).json();
    this.contractJson = await (await fetch("./BrokerlessContract.json")).json();
    this.leaseJson = await (await fetch("./BrokerlessLease.json")).json();
    this.brokerlessJson = await (await fetch("./brokerless.json")).json();
    this.interestJson = await (await fetch("./BankInterestGenerator.json")).json();


    this.marketContract = new window.web3.eth.Contract(
      this.marketJson,
      this.contractAddress,
      { defaultAccount: this.accounts[0] }
    );
    console.log("Contract object", this.marketContract);

    this.isAdmin = this.accounts[0] == await this.marketContract.methods.owner().call();
